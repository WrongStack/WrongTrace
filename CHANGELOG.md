...
- **IPC Validation and Socket Lifecycle Bounds (`f64535b`, `fe63a06`)**: the `report_file_read` handler validated `file_path` but not `repo_name`, while `file_read_events.repo_name` is declared `NOT NULL` — so an otherwise well-formed event produced a database constraint violation instead of a clean JSON-RPC `-32602`. Socket lifecycle paths are now bounded.
